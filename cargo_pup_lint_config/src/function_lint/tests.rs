...
mod tests {
    ...
    use crate::function_lint::{FunctionLint, FunctionRule};
    use cargo_pup_common::project_context::{ProjectContext, ModuleInfo, TraitInfo};
    ...
    fn test_function_lint_generate_from_empty_contexts() {
        ...
        let function_lints: Vec<_> = lints.into_iter()
        ...
        let has_function_length_lint = function_lints.iter()
        ...
        let has_result_error_lint = function_lints.iter()
    ...
    fn test_function_lint_generate_from_contexts() {
        ...
        let has_nested_module_lint = function_lints.iter()
            .any(|lint| lint.name.contains("test_crate::module3::submodule::subsubmodule"));
        assert!(has_nested_module_lint, "Should have a lint for the deeply nested module");
    }
    ...
    fn test_function_lint_with_multiple_contexts() {
        ...
        let has_crate2_nested_module_lint = function_lints.iter()
            .any(|lint| lint.name.contains("crate2::module2::submodule::nested"));
        assert!(has_crate2_nested_module_lint, "Should have a lint for the deeply nested module from crate2");
        
        // Verify we do not have a duplicate lint for any module
        let unique_names: std::collections::HashSet<_> = function_lints.iter()
            .map(|lint| &lint.name)
            .collect();
        assert_eq!(unique_names.len(), function_lints.len(), "Should not have duplicate lint names");
    }
}