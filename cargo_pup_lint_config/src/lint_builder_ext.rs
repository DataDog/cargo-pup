...
fn run_command(lint_builder: &LintBuilder, command: &str, args: &[&str]) -> Result<Output> {
    ...
    let temp_file =
        NamedTempFile::new().context("Failed to create temporary configuration file")?;
    let config_path = temp_file.path().to_path_buf();

    // Write the configuration to the temporary file
    lint_builder
        .write_to_file(config_path.to_str().unwrap())
    ...
    let output = cmd.output().context("Failed to execute cargo-pup")?;
...
"#,
        ...
        assert!(
            ...
            "Validation should pass for valid cargo project"
        );
        ...
        let result = run_command(
            ...
            &[
                "--manifest-path",
                cargo_project_dir.join("Cargo.toml").to_str().unwrap(),
            ],
        );
...
"#,
        ...
        assert!(
            validation_result.is_ok(),
            "Validation should pass for Cargo.toml file path"
        );
    ...
    fn test_temp_config_file_creation() {
        ...
        assert!(
            write_result.is_ok(),
            "Should be able to write config to temp file"
        );
        ...
        let config_contents =
            std::fs::read_to_string(config_path).expect("Should be able to read temp config file");

        // Verify the config contains our rules
        assert!(
            config_contents.contains("test_rule_1"),
            "Config should contain test_rule_1"
        );
        assert!(
            config_contents.contains("test_rule_2"),
            "Config should contain test_rule_2"
        );
        assert!(
            config_contents.contains("lints"),
            "Config should contain lints field"
        );

        // Test that the config is valid RON format
        assert!(
            config_contents.trim().starts_with("("),
            "Config should start with RON tuple syntax"
        );
        assert!(
            config_contents.trim().ends_with(")"),
            "Config should end with RON tuple syntax"
        );
    }
